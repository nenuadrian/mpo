...
class MPOLoss(nn.Module):
    ...
    def _compute_components(
        ...
        """Internal helper: compute policy loss, kl penalties, dual loss & stats.

        If detach_duals=True, dual parameters are used as detached tensors so that
        no gradients flow into MPO dual parameters during M-step computation.
        Returns: (loss_policy, loss_kl_penalty, loss_dual, loss_temperature,
                  alpha_mean, alpha_stddev, stats)
        """
        # Project dual variables' log-values (in-place, no grads).
        with torch.no_grad():
            min_temp = self.min_log_temperature.to(
                device=self.log_temperature.device, dtype=self.log_temperature.dtype
            )
            self.log_temperature.data.clamp_(min=min_temp)

            min_alpha = self.min_log_alpha.to(
                device=self.log_alpha_mean.device, dtype=self.log_alpha_mean.dtype
            )
            self.log_alpha_mean.data.clamp_(min=min_alpha)
            self.log_alpha_stddev.data.clamp_(min=min_alpha)

            if self._action_penalization:
                min_pen = self.min_log_temperature.to(
                    device=self._log_penalty_temperature.device,
                    dtype=self._log_penalty_temperature.dtype,
                )
                self._log_penalty_temperature.data.clamp_(min=min_pen)

        # Use detached copies of log-params if requested (avoid gradients into duals).
        log_temperature = (
            self.log_temperature.detach() if detach_duals else self.log_temperature
        )
        log_alpha_mean = (
            self.log_alpha_mean.detach() if detach_duals else self.log_alpha_mean
        )
        log_alpha_stddev = (
            self.log_alpha_stddev.detach() if detach_duals else self.log_alpha_stddev
        )
        log_penalty_temperature = (
            self._log_penalty_temperature.detach()
            if (self._action_penalization and detach_duals)
            else getattr(self, "_log_penalty_temperature", None)
        )

        # Transform dual variables from log-space using softplus, add epsilon for safety.
        temperature = F.softplus(log_temperature) + _MPO_FLOAT_EPSILON  # [1]
        alpha_mean = F.softplus(log_alpha_mean) + _MPO_FLOAT_EPSILON  # [D] or [1]
        alpha_stddev = F.softplus(log_alpha_stddev) + _MPO_FLOAT_EPSILON  # [D] or [1]
    ...
    def forward(
        ...
        # For backward compatibility: return full loss and stats.
        loss_policy, loss_kl_penalty, loss_dual, _, _, _, stats = (
        ...
        return (loss_policy + loss_kl_penalty + loss_dual), stats
    ...
    def e_step_loss(
        ...
        """Compute the dual-only loss used in the E-step (optimize duals)."""
        _, _, loss_dual, _, _, _, stats = self._compute_components(
            actions,
            q_values,
            online_mean,
            online_scale,
    ...
    def m_step_loss(
        ...
        """Compute the policy/kl loss used in the M-step. If detach_duals=True the dual params are treated as constants."""
        loss_policy, loss_kl_penalty, _, _, _, _, stats = self._compute_components(
            actions,
            q_values,
            online_mean,
            online_scale,
        ...
        return (loss_policy + loss_kl_penalty), stats
...
class MPOAgent:
    def __init__(
        ...
        e_steps: int = 1,
        m_steps: int = 1,
        target_update_tau: float = 1.0,  # 1.0 -> hard copy, <1.0 -> soft updates (Polyak)
        dual_clip_norm: Optional[float] = None,
    ):
        self.device = device
        ...
        self.mpo_loss = MPOLoss(
            ...
            epsilon_mean=2.5e-3,
            epsilon_stddev=1e-6,
            init_log_temperature=0.02,
            init_log_alpha_mean=0.02,
            init_log_alpha_stddev=70.0,
        ).to(device)
        ...
        # New: alternating-solver options
        self.e_steps = int(e_steps)
        self.m_steps = int(m_steps)
        self.target_update_tau = float(target_update_tau)
        self.dual_clip_norm = (
            float(dual_clip_norm) if dual_clip_norm is not None else None
        )
    ...
    def learn_step(self):
        ...
        # ---- Alternating optimization: E-step (dual) -> critic (TD) -> M-step (policy) ----

        # E-step: update dual variables using the dual-only loss.
        dual_stats = {}
        for _ in range(max(1, self.e_steps)):
            dual_loss, ds = self.mpo_loss.e_step_loss(
                sampled_actions, q_samples, online_mean, online_scale, t_mean, t_scale
            )
            with self._param_lock:
                self._dual_optimizer.zero_grad()
                dual_loss.backward()
                if self.dual_clip_norm is not None:
                    torch.nn.utils.clip_grad_norm_(
                        self.mpo_loss.parameters(), self.dual_clip_norm
                    )
                self._dual_optimizer.step()
            dual_stats = ds  # keep last for logging
        ...
        # M-step: update policy (policy_head) with duals treated as fixed (detach_duals=True).
        policy_stats = {}
        policy_m_loss = torch.tensor(0.0, device=self.device)
        for _ in range(max(1, self.m_steps)):
            policy_m_loss, ps = self.mpo_loss.m_step_loss(
                sampled_actions,
                q_samples,
                online_mean,
                online_scale,
            ...
            with self._param_lock:
                ...
                policy_m_loss.backward()
                if self.clipping_enabled:
                    torch.nn.utils.clip_grad_norm_(
                        list(self.policy_head.parameters()), self.clip_norm
                    )
                self._policy_optimizer.step()
            policy_stats = ps  # keep last for logging

        self._learn_steps += 1

        # periodic target updates (soft/hard depending on target_update_tau)
        if self._learn_steps % self.target_policy_update_period == 0:
            if self.target_update_tau >= 1.0:
                ...
                with torch.no_grad():
                    for k, p in self.policy_head.state_dict().items():
                        tp = self.target_policy_head.state_dict()[k]
                        tp.copy_(
                            self.target_update_tau * p
                            + (1.0 - self.target_update_tau) * tp
                        )

        if self._learn_steps % self.target_critic_update_period == 0:
            # critic & obs encoder target update (apply tau similarly)
            if self.target_update_tau >= 1.0:
                self.target_critic.critic.load_state_dict(
                ...
                with torch.no_grad():
                    # critic body
                    for k, p in self.critic.critic.state_dict().items():
                        tp = self.target_critic.critic.state_dict()[k]
                        tp.copy_(
                            self.target_update_tau * p
                            + (1.0 - self.target_update_tau) * tp
                        )
                    # obs encoder
                    for k, p in self.obs_encoder.state_dict().items():
                        tp = self.target_obs_encoder.state_dict()[k]
                        tp.copy_(
                            self.target_update_tau * p
                            + (1.0 - self.target_update_tau) * tp
                        )

        # Merge stats for logging (include dual and policy stats)
        fetches = {
            "policy_loss": float(policy_m_loss.item()),
        ...
        for k, v in dual_stats.items():
            fetches["dual_" + k] = (
                float(v.detach().cpu().item()) if isinstance(v, torch.Tensor) else v
            )
...
def train(
    ...
    policy_sync_interval: int,
    clip_norm: float = 40.0,
    # NEW args threaded into MPOAgent
    e_steps: int = 1,
    m_steps: int = 1,
    ...
    agent = MPOAgent(
        ...
        clip_norm=clip_norm,
        # thread new options through to the agent
        e_steps=e_steps,
        m_steps=m_steps,
        target_update_tau=target_update_tau,
        dual_clip_norm=dual_clip_norm,
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--e_steps", type=int, default=1, help="Number of E-step iterations"
    )
    parser.add_argument(
        "--m_steps", type=int, default=1, help="Number of M-step iterations"
    )
    parser.add_argument(
        "--target_update_tau",
        type=float,
        default=1.0,
        help="Tau for soft target updates (1.0 = hard update, <1.0 = soft update)",
    )
    parser.add_argument(
        "--dual_clip_norm",
        type=float,
        default=None,
        help="Gradient clipping max-norm for dual updates; set to 0 to disable clipping.",
    )
...
if __name__ == "__main__":
    ...
    train(
        ...
        dual_clip_norm=args.dual_clip_norm,
    )
