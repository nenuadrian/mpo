...
def compute_weights_and_temperature_loss(
    ...
    """
    V-MPO variant of compute_weights_and_temperature_loss:
      - optionally subtract per-state baseline (mean over action samples) to get advantages,
        then temper with temperature (no grad to q_values/advantages),
    ...
    """
    # Optionally form advantages by subtracting mean across action samples per batch.
    if use_baseline:
        # q_values: [N, B] -> baseline shape [1, B]
        baseline = q_values.mean(dim=0, keepdim=True)
        tempered_q_values = (q_values.detach() - baseline.detach()) / temperature
    else:
...
class VMPOLoss(nn.Module):
    """
    PyTorch V-MPO loss: same as MPOLoss but uses advantage-based tempering in the E-step
    (handled via compute_weights_and_temperature_loss(use_baseline=True)).
    """
    ...
    def forward(
        ...
        # E-step: compute normalized weights & temperature loss using advantages (V-MPO)
        ...
        # Optional MO-MPO penalty (unchanged, no baseline for cost)
        if self._action_penalization:
            penalty_temperature = (
...
class MixtureOfGaussiansHead(nn.Module):
    """Outputs parameters for a Mixture of Gaussians policy."""
    ...
    def __init__(
        ...
        self.mean_layer = nn.Linear(input_dim, num_components * action_dim)
        self.log_scale_layer = nn.Linear(input_dim, num_components * action_dim)
        self.logits_layer = nn.Linear(input_dim, num_components)

        with torch.no_grad():
            nn.init.uniform_(self.mean_layer.weight, a=-1e-4, b=1e-4)
            nn.init.uniform_(self.log_scale_layer.weight, a=-1e-4, b=1e-4)
            nn.init.uniform_(self.logits_layer.weight, a=-1e-4, b=1e-4)
            if self.mean_layer.bias is not None:
                self.mean_layer.bias.zero_()
            if self.log_scale_layer.bias is not None:
                self.log_scale_layer.bias.zero_()
            if self.logits_layer.bias is not None:
                self.logits_layer.bias.zero_()

    def forward(self, inputs):
        B = inputs.shape[0]
        means = self.mean_layer(inputs).view(B, self.num_components, self.action_dim)
        if self.tanh_mean:
            means = torch.tanh(means)
        log_scales = self.log_scale_layer(inputs).view(
            B, self.num_components, self.action_dim
        )
        scales = (
            F.softplus(log_scales)
            * (self.init_scale / F.softplus(torch.zeros(1, device=inputs.device)))
            + self.min_scale
        )
        logits = self.logits_layer(inputs)  # (B, num_components)
        return means, scales, logits

    def get_distribution(self, means, scales, logits):
        # means/scales: (B, K, D), logits: (B, K)
        mix = dist.Categorical(logits=logits)
        comp = dist.Independent(dist.Normal(means, scales), 1)
        return dist.MixtureSameFamily(mix, comp)


def mog_diag_normal_kl(p_means, p_scales, p_logits, q_means, q_scales, q_logits):
    """
    KL( MoG(p) || MoG(q) ) using MC samples from p.
    All shapes: [B, K, D] for means/scales, [B, K] for logits.
    Returns: [B] (mean over MC samples)
    """
    B, K, D = p_means.shape
    # Sample from p
    mix = dist.Categorical(logits=p_logits)
    comp = dist.Independent(dist.Normal(p_means, p_scales), 1)
    mog_p = dist.MixtureSameFamily(mix, comp)
    samples = mog_p.sample((16,))  # [S, B, D]  # <-- changed from rsample to sample
    # log probs under p and q
    log_p = mog_p.log_prob(samples)  # [S, B]
    mog_q = dist.MixtureSameFamily(
        dist.Categorical(logits=q_logits),
        dist.Independent(dist.Normal(q_means, q_scales), 1),
    )
    log_q = mog_q.log_prob(samples)  # [S, B]
    kl = (log_p - log_q).mean(dim=0)  # [B]
    return kl


def compute_mog_cross_entropy_loss(sampled_actions, normalized_weights, mog_dist):
    # sampled_actions: [N, B, D], normalized_weights: [N, B]
    N, B, D = sampled_actions.shape
    log_prob = mog_dist.log_prob(sampled_actions)  # [N, B]
    loss = -torch.sum(log_prob * normalized_weights, dim=0)  # [B]
    return loss.mean()


class MOGMPOLoss(nn.Module):
    """
    MPO loss for Mixture of Gaussians policies.
    """
    ...
    def forward(
        ...
        actions: torch.Tensor,  # [N, B, D]
        q_values: torch.Tensor,  # [N, B]
        online_means: torch.Tensor,  # [B, K, D]
        online_scales: torch.Tensor,  # [B, K, D]
        online_logits: torch.Tensor,  # [B, K]
        target_means: torch.Tensor,  # [B, K, D]
        target_scales: torch.Tensor,  # [B, K, D]
        target_logits: torch.Tensor,  # [B, K]
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        dtype = q_values.dtype
        with torch.no_grad():
            self.log_temperature.data = torch.maximum(
        ...
        # KL between non-parametric and target (diagnostic)
        kl_nonparametric = compute_nonparametric_kl_from_normalized_weights(
            normalized_weights
        )
        penalty_kl_nonparametric = None
        ...
        if self._action_penalization:
            ...
            diff_out_of_bound = actions - actions.clamp(-1.0, 1.0)
            cost_out_of_bound = -diff_out_of_bound.norm(dim=-1)
            penalty_normalized_weights, loss_penalty_temperature = (
                compute_weights_and_temperature_loss(
                    q_values=cost_out_of_bound,
                    epsilon=self._epsilon_penalty,
        ...
        # Build MoG distributions
        mog_online = dist.MixtureSameFamily(
            dist.Categorical(logits=online_logits),
            dist.Independent(dist.Normal(online_means, online_scales), 1),
        )
        mog_target = dist.MixtureSameFamily(
            dist.Categorical(logits=target_logits),
            dist.Independent(dist.Normal(target_means, target_scales), 1),
        )

        # Cross-entropy (M-step) terms
        loss_policy = compute_mog_cross_entropy_loss(
            actions, normalized_weights, mog_online
        )

        # KL computations: target || online (MC estimate)
        kl = mog_diag_normal_kl(
            p_means=target_means,
            p_scales=target_scales,
            p_logits=target_logits,
            q_means=online_means,
            q_scales=online_scales,
            q_logits=online_logits,
        )
        loss_kl, loss_alpha = compute_parametric_kl_penalty_and_dual_loss(
            kl=kl, alpha=alpha_mean, epsilon=self._epsilon_mean
        )
        ...
        stats["loss_policy"] = loss_policy.detach()
        stats["total_loss"] = loss.detach()
        stats["loss_alpha"] = loss_alpha.detach()
        stats["loss_temperature"] = loss_temperature.detach()
        stats["kl_q_rel"] = kl_nonparametric.mean() / self._epsilon
        if self._action_penalization and penalty_kl_nonparametric is not None:
            stats["penalty_kl_q_rel"] = (
        ...
        stats["kl_rel"] = kl.mean() / self._epsilon_mean
...
class MPOAgent:
    def __init__(
        ...
        ratio_tolerance=0.1,
        num_components=5,  # NEW: number of mixture components
    ):
        self.device = device
        ...
        # Mixture of Gaussians policy head
        self.policy_head = MixtureOfGaussiansHead(
            policy_hidden[-1],
            action_dim,
    ...
    def select_action(self, obs: np.ndarray, stochastic: bool = True) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            means, scales, logits = self.policy_head(emb)
            ...
            if stochastic:
                ...
                # Use mean of the most probable component
                idx = logits.argmax(dim=-1)
                action = means[0, idx[0]]
    ...
    def learn_step(self):
        ...
        with torch.no_grad():
            ...
            t_means, t_scales, t_logits = self.target_policy_head(
                emb_next
            )  # (B,K,D), (B,K,D), (B,K)
            N = self.num_samples
            # sample N actions per batch element from target MoG
            mog_target = dist.MixtureSameFamily(
                dist.Categorical(logits=t_logits),
                dist.Independent(dist.Normal(t_means, t_scales), 1),
            )
            sampled_actions = mog_target.sample((N,))  # (N,B,D)
        ...
        online_means, online_scales, online_logits = self.policy_head(emb_o_t)
...
def run_policy_modules(
    ...
    with torch.no_grad():
        ...
        means, scales, logits = policy_head(emb)
        mog = dist.MixtureSameFamily(
            dist.Categorical(logits=logits),
            dist.Independent(dist.Normal(means, scales), 1),
        )
...
def train(
    ...
    action_penalization: bool = True,
    per_dim_constraining: bool = True,
    num_components: int = 5,  # NEW: number of mixture components
):
    random.seed(seed)
    np.random.seed(seed)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of mixture components for Mixture of Gaussians policy.",
    )
